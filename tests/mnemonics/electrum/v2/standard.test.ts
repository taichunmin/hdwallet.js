...
describe('ElectrumV2Mnemonic (Standard)', () => {
  const mnemonicType = ELECTRUM_V2_MNEMONIC_TYPES.STANDARD;
  ...
  it('encodes & decodes each supported entropy → mnemonic → entropy', () => {
    ...
    const vectors = (jsonData['Electrum-V2'] as Vector[])
      .filter(v => v['mnemonic-types'].standard);

    for (const { words, 'mnemonic-types': m } of vectors) {
      const byLang = m.standard;
