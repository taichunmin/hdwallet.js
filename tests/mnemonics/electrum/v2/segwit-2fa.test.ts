...
describe("ElectrumV2Mnemonic", (): void => {
    ...
    for (const words of ElectrumV2Mnemonic.wordsList) {
      type Vector = {
        ...
        const suitable: Record<number, Record<string, string>> = Object.fromEntries(
          vectors.map(v => {
            const byLang = v['mnemonic-types']['segwit-2fa'];
      ...
      for (const language of ElectrumV2Mnemonic.languages) {
        ...
        const mnemonic: string = ElectrumV2Mnemonic.encode(
            entropy, language, { mnemonicType: ELECTRUM_V2_MNEMONIC_TYPES.SEGWIT_2FA }
        );
        expect(typeof mnemonic).toBe("string");
        expect(mnemonic.split(" ")).toHaveLength(words);
        expect(ElectrumV2Mnemonic.isValid(mnemonic, { mnemonicType: ELECTRUM_V2_MNEMONIC_TYPES.SEGWIT_2FA })).toBe(true);
        const decoded: string = ElectrumV2Mnemonic.decode(
            mnemonic, { mnemonicType: ELECTRUM_V2_MNEMONIC_TYPES.SEGWIT_2FA }
        );
  ...
  it("fromWords() should generate mnemonics of each supported length", (): void => {
    for (const words of ElectrumV2Mnemonic.wordsList) {
      const electrumV2Mnemonic: ElectrumV2Mnemonic = ElectrumV2Mnemonic.fromWords(
          words, ELECTRUM_V2_MNEMONIC_LANGUAGES.ENGLISH, { mnemonicType: ELECTRUM_V2_MNEMONIC_TYPES.SEGWIT_2FA }
      );
      ...
      expect(ElectrumV2Mnemonic.isValid(electrumV2Mnemonic.mnemonic(), { mnemonicType: ELECTRUM_V2_MNEMONIC_TYPES.SEGWIT_2FA })).toBe(true);
    }
  });
  ...
  it("fromEntropy() should accept hex, Uint8Array, or ElectrumV2Entropy and round-trip correctly", (): void => {
    ...
    const entropy = vec12['mnemonic-types']['segwit-2fa'][ELECTRUM_V2_MNEMONIC_LANGUAGES.ENGLISH]['entropy-suitable']
    ...
    for (const input of [entropy, entropyBytes, electrumV2Entropy]) {
      const electrumV2Mnemonic: ElectrumV2Mnemonic= ElectrumV2Mnemonic.fromEntropy(
          input, ELECTRUM_V2_MNEMONIC_LANGUAGES.ENGLISH, { mnemonicType: ELECTRUM_V2_MNEMONIC_TYPES.SEGWIT_2FA }
      );
      const decoded: string = ElectrumV2Mnemonic.decode(electrumV2Mnemonic.mnemonic(), { mnemonicType: ELECTRUM_V2_MNEMONIC_TYPES.SEGWIT_2FA });
      expect(decoded).toBe(entropy);
    }
  });
